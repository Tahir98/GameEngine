...
public:
	...
	void translate2(const float x,const float y,const float z) {
		position = position + GLMath::normalize(GLMath::crossProduct(direction, {0,1,0})) * x;

		Vec3 right = GLMath::normalize(GLMath::crossProduct(direction, {0,1,0}));
		Vec3 up = GLMath::normalize(GLMath::crossProduct(right, direction));
	...
	void translate2(const Vec3 trans) {
		position = position - GLMath::normalize(GLMath::crossProduct(direction, { 0,1,0 })) * trans.x;
