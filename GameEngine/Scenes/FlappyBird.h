...
public:
	...
	FlappyBird(GLFWwindow* window) :Scene(window), handler(window), background("assets/city.png") {
		bird.reserve(4);
		bee.reserve(2);
	...
	void render(float delta) {
		...
		if (state == GameState::START || state == GameState::GAME_LOOP) {
			...
			text->write(a.c_str(), Application::width / 20, Application::height - 50, 1, { 0,0,0 });
		}
		else if (state == GameState::GAME_OVER) {
			text->write("GAME OVER!!!", Application::width / 2.0f - Application::width / 10, Application::height / 2.0f, 1, { 0,0,0 });
		}

		
		if (isCollide) {
			text->write("Collided!!!!!!", 0, 0, 1, { 1,0,0 });
		}
